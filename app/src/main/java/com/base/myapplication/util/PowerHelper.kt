 ...
 *
 */
inline fun jsonObject(argsBuilder: JSONObject.() -> Unit): JSONObject =   JSONObject().apply(argsBuilder)
...
fun getCurrentDateWithFormat(outputDatePattern: String): String = doMagic(outputDatePattern, Calendar.getInstance().time)
...
fun getDateBeforeCurrentDate(outputDatePattern: String, number: Int): String = doMagic(outputDatePattern, doMagicCalendar(-number))

fun getDateAfterCurrentDate(outputDatePattern: String, number: Int): String = doMagic(outputDatePattern, doMagicCalendar(number))
...
fun String?.formatToRupiah() : String {
    return "Rp ${NumberFormat.getIntegerInstance(Locale.GERMANY).format(this.clearRupiahNumbering().toLong())}"
}

fun Int?.formatToRupiah() : String {
    return "Rp ${NumberFormat.getIntegerInstance(Locale.GERMANY).format(this.toString().clearRupiahNumbering().toLong())}"
}
...
fun String?.clearRupiah() : String {
    return NumberFormat.getIntegerInstance(Locale.GERMANY).format(this.clearRupiahNumbering().toLong())
}

fun Int?.clearRupiah() : String {
    return NumberFormat.getIntegerInstance(Locale.GERMANY).format(this.toString().clearRupiahNumbering().toLong())
}
...
fun String?.clearRupiahNumbering(): String {
    val replaceable = String.format("[Rp,.\\s]", NumberFormat.getCurrencyInstance(Locale("id", "ID"))).toRegex()
    return try {
...
fun <T: Fragment> T.setDrawable(drawable: Int): Drawable? = ContextCompat.getDrawable(requireContext(), drawable)
fun <T: Activity> T.setDrawable(drawable: Int): Drawable? = ContextCompat.getDrawable(this, drawable)
